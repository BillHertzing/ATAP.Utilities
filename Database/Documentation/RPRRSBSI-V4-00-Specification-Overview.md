...
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| D-1 GUID text              | Emit canonical lowercase `D`-format GUID text in CSV and APIs; SQL Server equality remains binary-value based.                            | Loader formatting and API tests.       |
| D-3 declared type change   | A materially different declared rule/input/output type creates a new basic identity rather than mutating meaning in place.                | Migration and compatibility policy.    |
| D-4 overlay representation | An override is a `RuleVariant` tied to the same basic `RuleId`, selected through an occurrence row with membership role `Override`.       | Core tables and resolution query.      |
