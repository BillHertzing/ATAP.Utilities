...
function Get-CSharpPackageAuthenticodeReleaseContract {
  ...
  if ($packages.Count -ne 45 -or $assets.Count -ne 119) { throw "The current release contract must contain exactly 45 packages and 119 shipping DLL assets; found $($packages.Count) packages and $($assets.Count) assets." }
...
function Get-CSharpPackageAuthenticodeApproval {
  ...
  switch ($taskId) {
    'triple-stream-csharp-signing-contract-45' {
      ...
      $expectedAssetCount = 119
      $scopeName = 'current 45-package/119-asset filter release slice'
    }
    '15.182.F03' {
