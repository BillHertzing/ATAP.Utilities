...
if (-not $(Test-Path -Path $GeneratedPowershellPackagesDestinationPath -PathType Container)) { $null = New-Item -ItemType Directory -Force $GeneratedPowershellPackagesDestinationPath }
...
Task BuildPSM1 -Inputs (Get-Item $(Join-Path $SourcePath '*' '*.ps1')) -Outputs $ModuleOutputPath {
    Write-PSFMessage -Level Important -Message 'task = BuildPSM1'
