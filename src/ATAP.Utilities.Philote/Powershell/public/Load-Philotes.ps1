<#
.SYNOPSIS
Loads Philotes data for an organization
.DESCRIPTION
Long description
...
The name of the database to load data into. Defaults to 'Philotes'.
...
.EXAMPLE
Load-Philotes -DatabaseHost 'localhost' -Environment 'Development' -DatabaseName 'Philotes' -IntegratedSecurity

.EXAMPLE
Load-Philotes -DatabaseHost 'localhost' -Environment 'Development' -IntegratedSecurity -SourcePath 'C:\Data\philotes.json' -BatchSize 50

.EXAMPLE
Load-Philotes -Environment 'Production' -CredentialsKey 'PhilotesDB-Prod' -BatchSize 200
...
function Load-Philotes {
  ...
  BEGIN {
    ...
    $privateFunction = Join-Path $PSScriptRoot '..\private\Import-Philotes.ps1'
    ...
    $Environment = Get-PVal -ParameterName 'Environment' -originalPSBoundParameters $PSBoundParameters -dottedPath 'DatabasesCollection.Philotes.Environment' -DefaultValue 'Experimental' -AllowMissing:$true -ValidValues @('Development', 'Testing', 'Production', 'Experimental')
    ...
    $DatabaseHost = Get-PVal -ParameterName 'DatabaseHost' -originalPSBoundParameters $PSBoundParameters -dottedPath "DatabasesCollection.Philotes.$Environment.DatabaseHost" -DefaultValue $DatabaseHost -AllowMissing:$true
    ...
    $SqlInstance = Get-PVal -ParameterName 'SqlInstance' -originalPSBoundParameters $PSBoundParameters -dottedPath "DatabasesCollection.Philotes.$Environment.SqlInstance" -DefaultValue $sqlInstanceDefault -AllowMissing:$true
    ...
    $DatabaseName = Get-PVal -ParameterName 'DatabaseName' -originalPSBoundParameters $PSBoundParameters -dottedPath 'DatabasesCollection.Philotes.DatabaseName' -DefaultValue 'Philotes' -AllowMissing:$true
    ...
    $ConnectionMethod = Get-PVal -ParameterName 'ConnectionMethod' -originalPSBoundParameters $PSBoundParameters -dottedPath 'DatabasesCollection.Philotes.ConnectionMethod' -DefaultValue 'default' -AllowMissing:$true -ValidValues @('tcp', 'np', 'lpc', 'default')
    ...
    if ($PSCmdlet.ParameterSetName -eq 'CredentialsFromVault') {
      $CredentialsKey = Get-PVal -ParameterName 'CredentialsKey' -originalPSBoundParameters $PSBoundParameters -dottedPath "DatabasesCollection.Philotes.$Environment.CredentialsKey" -DefaultValue $CredentialsKey -AllowMissing:$false
    ...
    $BatchSize = Get-PVal -ParameterName 'BatchSize' -originalPSBoundParameters $PSBoundParameters -dottedPath 'DatabasesCollection.Philotes.BatchSize' -DefaultValue 100 -AllowMissing:$true
  ...
  PROCESS {
    ...
    try {
      ...
      if ($PSCmdlet.ShouldProcess($DatabaseName, 'Load tag data')) {
        ...
        $result = Import-Philotes @importParams
      ...
      $errorMessage = "Load-Philotes failed: $($_.Exception.Message)"
      Write-PSFMessage -FunctionName $fn -ModuleName $mn -Level Error -Message $errorMessage
