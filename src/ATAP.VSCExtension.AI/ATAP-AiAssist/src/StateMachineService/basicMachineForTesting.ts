...
// **********************************************************************************************************************
// input type, context type, output type, events, event payload types, events union for machine, event for notification for completion for the querySingleEngineMachine
export interface IQuerySingleEngineMachineInput
  extends IAllMachinesBaseContext {
...
type QuerySingleEngineMachineAllEventsT =
  | QuerySingleEngineMachineStartEventsT
...
// **********************************************************************************************************************
// Parameter type for the NotifyCompleteAction
export interface IQuerySingleEngineMachineNotifyCompleteActionParameters {
  logger: ILogger;
  sendToTargetActorRef: ActorRef<any, any>;
  eventCausingTheTransitionIntoOuterDoneState: QuerySingleEngineMachineCompletionEventsT;
...
const nameOfMachine2 = setup({
  ...
  actions: {
    ...
    notifyCompleteAction: sendTo(
      ...
      (_, params: IQuerySingleEngineMachineNotifyCompleteActionParameters) => {
        ...
        // discriminate on event that triggers this action and send QUERY.SINGLE_ENGINE_MACHINE_DONE or DISPOSE_COMPLETE
        let _eventToSend:
          | {
              type: "QUERY.SINGLE_ENGINE_MACHINE_DONE";
              payload: IQuerySingleEngineMachineDoneEventPayload;
  ...
  //cspell:ignore N4IgpgJg5mDOIC5gF8A0IB2B7CdGgwEMBbMAeQDN8QAHLWASwBcGsNqAPRAWgDZ0Anj17I0mEuQoA6BhmYNCAGwDKTQkzDU6jFm06IALACZBiABwBGKQAYDAVgsBmXkYDsjs7YtHRooA
  ...
  context: ({ input }) => ({
    logger: new Logger(input.logger, "testMachine"),
    parent: input.parent,
  ...
  id: "testMachine_basicMachineForTesting",
  ...
  states: {
    querySingleEngineState: {
      ...
      entry: [
        ({ context }) => {
          context.logger.log(
            `querySingleEngineMachine querySingleEngineState entry`,
            LogLevel.Debug,
          );
        },
        "debugMachineContext",
    ...
    doneState: {
      ...
      entry: [
        // call the notifyComplete action here, setting the value of the params' properties to values pulled from the context
        ...
        //  set into params here, when the lambdas run
        {
          ...
          params: ({ context, event }) =>
            ({
              logger: new Logger(context.logger, "notifyCompleteAction"),
              // logger: context.logger,
