function Remove-DeveloperDatabaseInstances {
  ...
  At the end of a sprint, removes the two named instances that were created per developer:
    - Sprint{NNNN}{DeveloperName}   — sprint-scoped instance
    - Development{DeveloperName}    — developer's persistent instance
  ...
  foreach ($developer in $DeveloperNames) {
    $instances = @(
      [PSCustomObject]@{ InstanceLabel = 'Sprint'; SqlInstance = "Sprint${SprintNumber}${developer}" }
      [PSCustomObject]@{ InstanceLabel = 'Development'; SqlInstance = "Development${developer}" }
    )
