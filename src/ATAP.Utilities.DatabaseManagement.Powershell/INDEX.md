...
| Cmdlet                     | File                                                                       | Synopsis                                                                                                                                                                                                                                                                              |
| -------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Build-DatabaseWithFlyway` | [public/Build-DatabaseWithFlyway.ps1](public/Build-DatabaseWithFlyway.ps1) | Orchestrates a complete database build: drops and recreates the database via source-loaded active worktree helpers, then applies all Flyway migrations. Accepts `-SqlInstance` directly (e.g. `Devwhertzing`, `Expwhertzing`). When it calls `Invoke-Flyway` with a resolved `SqlConnection`, the connection object carries authentication state instead of combining mutually exclusive Flyway parameters. Primary cmdlet for creating ATAPUtilities and AceCommander databases. |
