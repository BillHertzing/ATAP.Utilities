...
BeforeAll {
  ...
  . "$PSScriptRoot\..\..\private\Initialize-ATAPConfigurationGlobals.ps1"
}
...
Describe 'Initialize-ATAPConfigurationGlobals' -Tag 'Unit' {
  BeforeEach {
    ...
    $script:tempRepositoryRoot = Join-Path ([System.IO.Path]::GetTempPath()) "atap-config-bootstrap-$([guid]::NewGuid().ToString('N'))"
...
'@
  ...
  It 'fails when host settings omit the required database collection' {
