...
| [Plans/Invoke-CSharpPackageBuildMasterStage.ps1](Plans/Invoke-CSharpPackageBuildMasterStage.ps1)                           | `Invoke-CSharpPackageBuildMasterStage`   | Drives the C# five-tier pipeline. Experimental builds with the pinned .NET SDK, resolves stable Visual Studio Build Tools MSBuild 18.8+ and SDK-selected NuGet Pack 7.8+, applies the Git-derived deterministic timestamp, and publishes only after deterministic verification; later tiers promote the immutable package. |
