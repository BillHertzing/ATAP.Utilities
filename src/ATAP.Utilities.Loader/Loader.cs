...
namespace ATAP.Utilities.Loader {
  ...
  public class Loader<IType> {
    public void LoadExactlyOneInstanceOfITypeFromAssemblyGlobAsSingleton(IDynamicGlobAndPredicate dynamicGlobAndPredicate = default, IServiceCollection services) {
    ...
    public IType LoadExactlyOneInstanceOfITypeFromAssemblyGlob(IDynamicGlobAndPredicate dynamicGlobAndPredicate = default) {
      ...
      var initialInstance = (IType)Activator.CreateInstance(singleType);
      ...
      if (hasSubModules) {
        ...
        // Iterate
        foreach (var subModuleIType in subTypesToFunctionCriteriaDictionary.Keys) {
          var subModuleDynamicGlobAndPredicate = subTypesToFunctionCriteriaDictionary[subModuleIType];
