...
$ChocolateyPackagesForGroupNameScriptBlock = {
  for ($groupNameIndex = 0; $groupNameIndex -lt $groupNames.count; $groupNameIndex++) {
    ...
    if ($($parsedInventory.GroupNames[$groupName]).ContainsKey('ChocolateyPackageNames')) { # process for $groupName only if the ChocolateyPackageNames key exists
      [System.Text.StringBuilder]$sb = [System.Text.StringBuilder]::new()
      [void]$sb.Append(@"
- name: Install Chocolatey Packages For $groupName
  hosts: all
  gather_facts: false
  tasks:
    - name: Load Chocolatey Package information JSON file
      set_fact:
        resource_properties: "{{ lookup('file', '/mnt/c/Dropbox/whertzing/GitHub/ATAP.IAC/chocolateyPackageInfo.json') | from_json }}"
    - name: Install the Chocolatey Packages defined for each group
      win_dsc:
        resource_name: cChocoPackageInstaller
        Name: "{{ item }}"
        Version: "{{ resource_properties[item]['Version'] }}"
...
- name: Install roles For $groupName
  ...
  gather_facts: false
  tasks:
    - name: Run the following role's tasks on all the hosts in $groupName
      include_role:
        name: "{{ item }}"
      loop:
"@)
