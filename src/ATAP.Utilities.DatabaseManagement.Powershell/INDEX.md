...
| Cmdlet                                    | File                                                                                                     | Synopsis                                                                                                                                                                                                                        |
| ----------------------------------------- | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Resolve-DatabaseSqlConnection`           | [public/Resolve-DatabaseSqlConnection.ps1](public/Resolve-DatabaseSqlConnection.ps1)                     | Resolves the three supported database connection inputs to one open `Microsoft.Data.SqlClient.SqlConnection`: existing connection, Bitwarden connection-string secret, or `Get-PVal`-resolved connection parts. For the secret path (Task 9.22) the private helper resolves in order: (a) real vault secret, (b) deterministic build when the name is derivable via `Get-DbConnectionStringSecretDescriptor`, (c) hard fail if credentialed and absent. |
