...
.DESCRIPTION
    ...
    handles the mechanical file updates and Git operations afterward:
      ...
      6. Discovers all planning artifacts (Modernization Plan, TASKS.md, the
         sprint task HTML set, Sprint-Duration-Log.md, AI Conversation Analysis,
         Conversation Bookmark, README.md) and includes them in the commit
...
"@
        ...
        $planningArtifacts = @(
          ...
          (Join-Path $workDir 'TASKS.html'),
          (Join-Path $workDir 'Tasks.Accomplished.html'),
          (Join-Path $workDir 'Tasks.ProceduralDetails.html'),
        ...
        if (Test-Path -LiteralPath $workDir -PathType Container) {
          Get-ChildItem -LiteralPath $workDir -Filter 'TASKS_V*.html' -File -ErrorAction SilentlyContinue |
...
"@
        ...
        if ($adopted.Count -gt 0) {
          ...
          Write-PSFMessage -FunctionName $fn -ModuleName $mn -Level Important -Message '2. Review the active sprint board (`TASKS.html` or highest `TASKS_V*.html`) and confirm it matches `TASKS.md`.'
          Write-PSFMessage -FunctionName $fn -ModuleName $mn -Level Important -Message '3. Verify `Tasks.Accomplished.html` and `Tasks.ProceduralDetails.html` were created or updated as needed for the adopted work.'
          Write-PSFMessage -FunctionName $fn -ModuleName $mn -Level Important -Message '4. Verify Conversation Bookmark was generated: AceCommander_Project_State_Conversation_Bookmark_NNN.md'
          Write-PSFMessage -FunctionName $fn -ModuleName $mn -Level Important -Message '5. Commit all planning artifacts and create PR from the sprint branch.'
        } else {
          Write-PSFMessage -FunctionName $fn -ModuleName $mn -Level Verbose -Message 'No ideas were adopted. Check TASKS.md, the active task board, and the companion task HTML files for any manual edits before creating the planning PR.'
        }
