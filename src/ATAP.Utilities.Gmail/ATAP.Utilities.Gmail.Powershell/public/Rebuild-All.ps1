# Build-All script for Gmail database
# This script builds the Gmail database and loads initial data
...
try {
  ...
  Load-Gmail `
    ...
    -Environment $environment `
    -IntegratedSecurity `
    -ExtractPath 'D:/Temp/LoadGmail' `
    -TakeoutZipPath 'C:\Dropbox\Apps\Google Download Your Takeout Data\takeout-20251021T153420Z-1-001 mail drafts.zip' `
    -Verbose:$VerbosePreference

  Write-PSFMessage -Level Important -Message "=== $databaseName Database Build Complete ==="
}
catch {
  Write-PSFMessage -Level Error -Message "Load-Gmail failed: $($_.Exception.Message)"
  throw
}
