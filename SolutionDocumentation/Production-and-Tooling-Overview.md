...
| `SharedVSCode-PowerShell-Functions.md`   | `Import-SharedVSCodeFunctions.ps1`; `New-SprintStage1` / `New-SprintStage2` (function definition files — callers must `Import-Module ATAP.Utilities.BuildTooling.PowerShell -Force` or dot-source before calling); `Set-WorktreeJunctions`; `Invoke-WithFileLock`; transition plan to publish as a ProGet-hosted module. | gap                                                                                                                                                                                                                                         |
