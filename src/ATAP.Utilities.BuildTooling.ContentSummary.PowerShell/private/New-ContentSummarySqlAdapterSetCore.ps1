function New-ContentSummarySqlAdapterSetCore {
  [CmdletBinding()]
  param(
    ...
    [Parameter(Mandatory = $true)]
    [object] $StoredProcedureInvoker
  )

  begin {
    $fn = 'New-ContentSummarySqlAdapterSetCore'
  ...
  process {
    ...
    $invokeStoredProcedure = $StoredProcedureInvoker
    $convertCanonicalOrigin = Get-Command -Name 'ConvertTo-ContentSummaryCanonicalOriginUri' -CommandType Function -ErrorAction Stop
    $convertParameters = Get-Command -Name 'ConvertTo-ContentSummarySqlParameterDefinitions' -CommandType Function -ErrorAction Stop
    $convertDependencies = Get-Command -Name 'ConvertTo-ContentSummaryDependencyDataTable' -CommandType Function -ErrorAction Stop
    $assertCaptureAcknowledgement = Get-Command -Name 'Assert-ContentSummaryCaptureAcknowledgement' -CommandType Function -ErrorAction Stop
    ...
    $provisionRepository = {
      ...
      $canonicalOrigin = & $convertCanonicalOrigin -OriginUri $OriginUri
    ...
    $assignVersionTag = {
      ...
      $parameters = & $convertParameters -Contract AssignVersionTag -Values $values
      $result = & $invokeStoredProcedure -SqlConnection $connection -ProcedureName 'ATAPUtilities.AssignContentSummaryVersionTagV1' -Parameters $parameters -ResultPropertyOrder @('TagId','TagAssignmentId','ContentSummaryVersionId','StatusCode','ErrorCode') -AllowedStatusCodes @('Assigned','Existing') -CommandTimeoutSeconds $timeout
    ...
    $authorizeRepository = {
      ...
      $parameters = & $convertParameters -Contract AuthorizeRepository -Values $values
      $result = & $invokeStoredProcedure -SqlConnection $connection -ProcedureName 'ATAPUtilities.AuthorizeContentSummaryDatabasePrincipalRepositoryV1' -Parameters $parameters -ResultPropertyOrder @('AuthorizationId','DatabasePrincipalName','InstanceCode','RepositoryId','StatusCode','ErrorCode') -AllowedStatusCodes @('Authorized','Existing') -CommandTimeoutSeconds $timeout
    ...
    $capture = {
      ...
      $dependencyTable = & $convertDependencies -Dependencies $Dependencies
      ...
      $parameters = & $convertParameters -Contract Capture -Values $values
      $result = & $invokeStoredProcedure -SqlConnection $connection -ProcedureName 'ATAPUtilities.CaptureContentSummaryObservationV1' -Parameters $parameters -ResultPropertyOrder @('ReplayStatus','IdempotencyKey','SourceArtifactId','SourceArtifactVersionId','ContentSummaryId','ContentSummaryVersionId','SourceArtifactVersionSequence','ContentSummaryVersionSequence','LifecycleCode','ErrorCode') -AllowedStatusCodes @('Created','Replayed','DerivationReplay') -StatusPropertyName ReplayStatus -CommandTimeoutSeconds $timeout
      & $assertCaptureAcknowledgement -Result $result -IdempotencyKey $IdempotencyKey -SourceArtifactId $SourceArtifactId -SourceArtifactVersionId $SourceArtifactVersionId -ContentSummaryId $ContentSummaryId -ContentSummaryVersionId $ContentSummaryVersionId
    }.GetNewClosure()
