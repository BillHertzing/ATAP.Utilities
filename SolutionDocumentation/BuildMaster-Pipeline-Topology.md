...
| `Invoke-PromotedPackageTests`      | C# pipeline (T2–T5)                          | Run `dotnet test` against the `.Tests.csproj` project with `UsePackageReferenceForSUT=true` and `SUTVersion` set to the promoted version; accepts `-TestFilter` for tier-appropriate `--filter` arguments. | spec        |
| `Invoke-PromotedModuleTests`       | PowerShell pipeline (T2–T5)                  | Run Pester against the installed promoted PowerShell module version from the tier's feed.                                                                                                                  | spec        |
