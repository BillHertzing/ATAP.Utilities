...
function Get-HostSettings {
  ...
  [OutputType([Object])]
  Param (
    # The hostname for the computer host for which settings will be created
    ...
    $hostName
    # the ansibleInventory file which defines the organizations IAC
  ...
  BEGIN {
    ...
    function ProcessBufferLocalGet-HostSettings {
      Write-PSFMessage -Level Debug -Message 'Starting Function %FunctionName% in module %ModuleName%' -Tag 'Trace'

      $script:inputObj

    }
    function RemoteGet-HostSettings {
      Write-PSFMessage -Level Debug -Message 'Starting Function %FunctionName% in module %ModuleName%' -Tag 'Trace'

      $script:inputObj

    }
    function DispatchToBufferGet-HostSettings {
      Write-PSFMessage -Level Debug -Message 'Starting Function %FunctionName% in module %ModuleName%' -Tag 'Trace'
  ...
  PROCESS {
    foreach ($script:inputObj in $input) {
      if ($script:inputObj.PSobject.Properties.Name -notcontains 'ParamName1') {
        ...
        for ($PropertyNamesIndex = 0; $PropertyNamesIndex -lt $PropertyNames.Count; $PropertyNamesIndex++) {
          ...
          switch ($PropertyName) {
            'ParamName1' { $hostName = $script:inputObj.PSobject.Properties['ParamName1'].value; break }
            default {
    ...
    # the $hostName argument is not null
    # DispatchToBuffer for further type checking and processing of the script:inputObj
    DispatchToBuffer
  }
}
