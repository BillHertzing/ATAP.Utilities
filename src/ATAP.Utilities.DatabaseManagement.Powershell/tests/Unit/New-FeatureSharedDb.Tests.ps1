...
# Pester 5+ tests for New-FeatureSharedDb (Stream J3).

BeforeAll {
  $publicDir = Join-Path (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)) 'public'
  . (Join-Path $publicDir 'Resolve-DbInstanceName.ps1')
  . (Join-Path $publicDir 'New-FeatureSharedDb.ps1')
...
Describe 'New-FeatureSharedDb' -Tag 'Unit' {
  BeforeEach {
  ...
  It 'creates the canonical feature shared database when absent' {
    $result = New-FeatureSharedDb -Application 'AceCommander' -FeatureSlug 'PaymentRefactor' -SqlInstance 'test-sql'
    ...
    Assert-MockCalled Invoke-DatabaseSqlNonQuery -Times 1 -Exactly -Scope It -ParameterFilter {
      $SqlConnection -eq $script:fakeConnection -and $CommandText.Contains('CREATE DATABASE [AceCommander-PaymentRefactor-shared]')
    }
  }
  ...
  It 'is idempotent when the database is already present' {
    ...
    $result = New-FeatureSharedDb -Application 'AceCommander' -Feature PaymentRefactor -SqlInstance 'test-sql'
  ...
  It 'accepts DBConnectionStringSecretName as its own parameter set' {
    New-FeatureSharedDb -Application 'AceCommander' -Feature PaymentRefactor -DBConnectionStringSecretName 'db-secret' | Out-Null
  ...
  It 'honors WhatIf without running CREATE DATABASE' {
    $result = New-FeatureSharedDb -Application 'AceCommander' -Feature PaymentRefactor -SqlInstance 'test-sql' -WhatIf

    $result.Created | Should -BeFalse
    $result.ResponseSummary | Should -Be "would create database 'AceCommander-PaymentRefactor-shared'"
    Assert-MockCalled Invoke-DatabaseSqlNonQuery -Times 0 -Exactly -Scope It
  }
}
