...
# Desired settings
$ShellExtKeyList = @{
  'Documents' = ('Personal', "$DropboxBase$DocumentsRelPath")
  'Favorites' = ('Favorites', "$DropboxBase$FavoritesRelPath")
  'Music'     = ('My Music', "$DropboxBase$MusicRelPath")
  'Photos'    = ('My Pictures', "$DropboxBase$PhotosRelPath")
...
# current settings
$cur = get-itemproperty -Path ($ShellExtRegKeyBase ) -Exclude PS*Path, PSDrive, PSChildName, PSProvider -name (($ShellExtKeyList.Values) | ForEach-Object { $_[0] })
...
if ($hasdiff.Count -gt 0) {
  $hasdiff.Keys | ForEach-Object { $key = $_;
...
#	      # Release the Commanded Proxies (or client channels)
