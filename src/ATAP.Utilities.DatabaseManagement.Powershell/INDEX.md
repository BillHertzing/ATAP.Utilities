...
| `Get-SqlServiceLoginGrantTarget`   | [public/Get-SqlServiceLoginGrantTarget.ps1](public/Get-SqlServiceLoginGrantTarget.ps1)     | Read-only topology/SID/role audit of explicitly admitted package targets; unrelated user databases are reported `NotApproved`. |
| `Set-SqlDatabasePackageDeploymentPrincipal` | [public/Set-SqlDatabasePackageDeploymentPrincipal.ps1](public/Set-SqlDatabasePackageDeploymentPrincipal.ps1) | Idempotent audit/grant/revoke for a frozen host-local BuildMaster SID and explicit database allow-list; supports `-WhatIf` and retains the login during rollback. |
...
| Cmdlet                   | File                                                                   | Synopsis                                                                                                                                                                                                                  |
| ------------------------ | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Invoke-SqlServerBackup` | [public/Invoke-SqlServerBackup.ps1](public/Invoke-SqlServerBackup.ps1) | Supports the Production-only `ATAPUtilities` pipeline at `localhost,50020`: checksum backup, authenticated compression/encryption, restore round-trip verification, and verified publication. |
| `Protect-SqlServerBackupArtifact` | [public/Protect-SqlServerBackupArtifact.ps1](public/Protect-SqlServerBackupArtifact.ps1) | Streams a backup through GZip and AES-256-CBC with encrypt-then-MAC HMAC-SHA256; resolves only `dbEncryption.ATAPUtilities.Production` through `Get-SecretATAP`. |
| `Restore-SqlServerBackupArtifact` | [public/Restore-SqlServerBackupArtifact.ps1](public/Restore-SqlServerBackupArtifact.ps1) | Authenticates the complete encrypted container before creating plaintext, then decrypts and decompresses it for restore verification. |
| `Publish-SqlServerBackupArtifact` | [public/Publish-SqlServerBackupArtifact.ps1](public/Publish-SqlServerBackupArtifact.ps1) | Publishes a verified encrypted SQL Server backup without overwriting an existing destination and retains the encrypted staged source for retention processing. |
| `Install-ProductionDatabaseBackupScheduledTasks` | [public/Install-ProductionDatabaseBackupScheduledTasks.ps1](public/Install-ProductionDatabaseBackupScheduledTasks.ps1) | Installs exactly two tasks: Sunday full and Monday-Saturday differential, fixed to Production `ATAPUtilities` and module 0.1.22. The local Windows identity is derived as `<host>\SvcSQLServer`; its host-suffixed SecretName supplies only the Task Scheduler password. The action keeps SQL transport encryption enabled and applies the approved certificate trust override only to the fixed local Production workflow. |
