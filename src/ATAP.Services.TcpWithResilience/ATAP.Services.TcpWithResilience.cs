...
using static Polly.Extensions.Http.HttpPolicyExtensions; // ToDo: Need a TCP focused extension to Polly for WaitAndRetryAsync and ExecuteAsync
...
namespace ATAP.Utilities.HostedServices.TcpWithResilienceHostedService {
  ...
  public class TcpWithResilience : ITcpWithResilience {
    ...
    // ToDo: Get the Policy parameters from an ICConfigurationRoot
    public static int retryCount = 3;
    public static TimeSpan pauseBetweenFailures = TimeSpan.FromMilliseconds(200);
    // public static Policy retryPolicy = new PolicyBuilder() // ToDo: need to figure this out
    //   .WaitAndRetryAsync(Backoff.ConstantBackoff(pauseBetweenFailures, retryCount: retryCount))
    //   .Build();
  ...
  public class TcpWithResilienceHostedService : IHostedService, IDisposable, ITcpWithResilienceHostedService {
    ...
    public TcpWithResilienceHostedServiceData HostedServiceTCPWithResilienceData { get; set; }

    public void Startup() {
      HostedServiceTCPWithResilienceData = new();
