#!/usr/bin/env python3
"""
Render script for Stick Figure Pirouette Scene.
Renders StickFigurePirouette to MP4 in _generated/ using Python 3.11 + Manim.
"""
import subprocess
import sys
import shutil
...
def render_pirouette(quality: str = "h") -> int:
    """
    Render the StickFigurePirouette scene.

    Args:
        quality: manim quality letter — 'l' (480p), 'm' (720p),
                 'h' (1080p), 'p' (4K), 'k' (8K).  Default is 'h'.
                 Passed as -q<letter> (Manim Community v0.18+ format).

    Returns:
        subprocess return code (0 = success).
    """
    print("=" * 70)
    print("Stick Figure Pirouette Renderer")
    print("=" * 70)
    ...
    cmd = [
        ...
        SCENE_NAME,
        f"-q{quality}",
        "--output_file", "StickFigurePirouette",
    ...
    if result.returncode == 0:
        stable_copy = OUTPUT_DIR / "StickFigurePirouette_latest.mp4"
...
if __name__ == "__main__":
    ...
    sys.exit(render_pirouette(quality=q))
