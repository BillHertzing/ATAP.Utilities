...
namespace ATAP.Utilities.HostedServices {
  ...
  [ETWLogAttribute]
#endif
  // A service that provides a collection of reactive fileWatchers
  public class FileWatchersHostedServiceData : IDisposable, IFileWatchersHostedServiceData {
    public FileWatchersHostedServiceData() : this(new List<IObservable<EventPattern<FileSystemEventArgs>>>()
      ) { }

    public FileWatchersHostedServiceData(List<IObservable<EventPattern<FileSystemEventArgs>>> fileWatchers) {
      FileWatchers = fileWatchers ?? throw new ArgumentNullException(nameof(fileWatchers));
    }

    public List<IObservable<EventPattern<FileSystemEventArgs>>> FileWatchers { get; }
  ...
  [ETWLogAttribute]
#endif
  public class FileWatchersHostedService : IHostedService, IDisposable, IFileWatchersHostedService {
    #region Common Constructor-injected fields from the GenericHost
    private readonly ILogger<FileWatchersHostedService> logger;
    ...
    #endregion
    public FileWatchersHostedService(
            // This service gets all the default injected services
            ILogger<FileWatchersHostedService> logger,
      ...
      this.stringLocalizer = null; // new StringLocalizer<FileWatchersHostedService>();
      this.logger.LogInformation("{@ .ctor}{FileWatchersHostedService} in .ctor");
      ...
      this.logger.LogInformation("{@ .ctor}{FileWatchersHostedService} leaving .ctor");
      // Initialize the Data structure
      fileWatchersHostedServiceData = new FileWatchersHostedServiceData();
    }

    // public data structure for this service
    // collection of observable fileWatchers
    public IFileWatchersHostedServiceData fileWatchersHostedServiceData { get; private set; }
    ...
    //  this.logger.LogInformation("FileWatchersHostedService.OnStop method called.");
    //  HostApplicationLifetime.StopApplication();
    //  base.OnStop();
    //}
    #endregion

    public static IObservable<string> ChangedFilesAsObservable() {
      return
          Observable
    ...
    // Useage
    //IDisposable ConsoleReadLineAsyncAsObservableDisposableHandle = ConsoleReadLineAsyncAsObservable().TakeMany(inputString => Action<string> ((inputString) => {DoSomething(inputstring);})).Subscribe(() => ConsoleReadLineAsyncAsObservableDisposableHandle.Dispose());
