  ...
  begin {
    ...
    foreach ($sectionFunction in $perDatabaseSectionFunctions) {
      ...
      if (Test-Path -LiteralPath $siblingPath -PathType Leaf) {
        Write-ConfigRootKeysMessage -FunctionName $fn -ModuleName $mn -Level Debug -Message "Dot-sourcing co-located sibling '$siblingPath'" -Tag 'ConfigRootKeys'
        . $siblingPath
        ...
        Write-ConfigRootKeysMessage -FunctionName $fn -ModuleName $mn -Level Debug -Message "Using already-loaded module-scoped '$sectionFunction' (built module; no co-located source)." -Tag 'ConfigRootKeys'
      }
    }
  }
  ...
  process {
    try {
      if ($PSCmdlet.ShouldProcess('$global:configRootKeys', 'Add database key constants')) {
        ...
        Write-ConfigRootKeysMessage -FunctionName $fn -ModuleName $mn -Level Verbose -Message 'Added core database key constants.'
      }
      ...
      foreach ($sectionFunction in $perDatabaseSectionFunctions) {
        if ($PSCmdlet.ShouldProcess('$global:configRootKeys', "Invoke $sectionFunction")) {
          Write-ConfigRootKeysMessage -FunctionName $fn -ModuleName $mn -Level Debug -Message "Invoking '$sectionFunction'" -Tag 'ConfigRootKeys'
          & $sectionFunction
          Write-ConfigRootKeysMessage -FunctionName $fn -ModuleName $mn -Level Verbose -Message "$sectionFunction completed."
        }
      }
    } catch {
