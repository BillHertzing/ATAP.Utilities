...
export const testMachine = setup({
  ...
  states: {
    querySingleEngineState: {
      ...
      entry: [
        ({ context }) => {
          context.logger.log(`querySingleEngineMachine querySingleEngineState entry`, LogLevel.Debug);
        },
        'debugMachineContext',
      ],
      invoke: {
  ...
  on: { QSE_START: { target: '.querySingleEngineState', actions: 'debugMachineContext' } },
});
