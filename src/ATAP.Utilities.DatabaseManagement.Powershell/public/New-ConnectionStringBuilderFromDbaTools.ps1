function New-ConnectionStringBuilderFromDbaTools {
  ...
  - VaultCredential: Retrieves complete connection info from vault (username, password, and optionally host/instance/port)
  ...
  .EXAMPLE
  New-ConnectionStringBuilderFromDbaTools -DatabaseHost 'localhost' -DatabaseName 'PCMSC' -IntegratedSecurity
  Uses Windows integrated authentication.

  .EXAMPLE
  New-ConnectionStringBuilderFromDbaTools -DatabaseName 'PCMSC' -CredentialsKey 'PCMSC_Dev_Credentials'
  Retrieves host, instance, username, password from vault.

  .EXAMPLE
  New-ConnectionStringBuilderFromDbaTools -DatabaseHost 'sqlserver01' -DatabaseName 'Gmail' -CredentialsKey 'Gmail_SQL_Creds' -AsJDBC
  ...
  param(
    ...
    [Parameter(Mandatory = $false, ParameterSetName = 'VaultCredential')]
    [string]$DatabaseHost,

    [Parameter(Mandatory = $true, ParameterSetName = 'IntegratedSecurity')]
    [Parameter(Mandatory = $true, ParameterSetName = 'VaultCredential')]
    [string]$DatabaseName,
  ...
  BEGIN {
    ...
    try {
      ...
      if (-not (Get-Command -Name 'Get-VaultSecret' -CommandType Function -ErrorAction SilentlyContinue)) {
        . 'C:\Dropbox\whertzing\GitHub\ATAP.Utilities\src\ATAP.Utilities.Security.Powershell\public\Get-VaultSecret.ps1'
      }
    ...
    try {
      ...
      switch ($PSCmdlet.ParameterSetName) {
        ...
        'VaultCredential' {
          ...
          $secret = Get-VaultSecret -VaultKey $CredentialsKey
