...
Every language-specific instruction file should instructions telling copilot to follow specific coding guidelines

- powershell: .github/instructions/PowerShell.instructions.md and with frontmatter applyTo: "\*_/_.ps1"

  - Coding Guidelines:
    - use approved verbs in the function names for public functions. private and internal functions do not need to follow this convention.
    - Log using `Write-PSFMessage`
    - Every `Write-PSFMessage` inside a function should include the first parameter ◦ `-FunctionName '<functionName>'` where the <functionName> is replaced by the name of the function
    - Every `Write-PSFMessage` inside a function should include the second parameter ◦ `-ModuleName '<moduleName>'` where the <moduleName> is replaced by the name of the module
    - never log using `-Level Info`, but rather use `-Level Important` for important messages
    ...
    - All calls to `Invoke-Command` should have a log message just before and just after the line that calls `Invoke-Command`. These log messages should have `-Level Debug`, and `-Tag 'InvokeCommandCall'`. The message for the log before the call is
      ````Powershell
           ...
           ```
      ````
    - wrap any code that might produce an exception in a try/catch/finally block. In the catch block, log the exception using the following structure, and add the finally block if it is not present
      ```Powershell
      catch {
      ...
      ```
    - Whenever a `catch` block appears in code to catch a possible exception, it should include a local variable `$_errorMessage` which includes a description of the operation that caused the error, and the message portion of the exception. The line setting the `$_errorMessage` should be followed by `Write-PSFMessage` having `-Level Error` and `-Message $_errorMessage`.
...
- You may use any dot net libraries or open source libraries in your responses.
- Your responses may include references to PowerShell Pro VSC extension features and capabilities
- When you have Powershell code elements to suggest, you will review open editor window that is loaded with a Powershell-language-specific snippets file located at C:\Users\whertzing\AppData\Roaming\Code\User\snippets\Powershell.json.
- You will give priority to resusing a snippet if one is appropriate. If you use a snippet, place the name of the snippet as a comment above the snippet body in your response
...
Whenever Copilots execute they understand the purpose of the Powershell globals and the ConffigRootKey. In C# all libraries understand they are part of a service that configures itself using Interfaces and DI.
...
The runtime provides a way to execute the functions / library entry points, and the rules and the state machine tell the runtime the order in whihc to call functions and the values of the variables expected by the function.
...
nstructions that provide a clear goal for CoPilot to take., each time copiklot is run.
