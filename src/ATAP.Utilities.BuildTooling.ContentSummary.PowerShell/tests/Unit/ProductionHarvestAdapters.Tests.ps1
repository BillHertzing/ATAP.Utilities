Describe 'ContentSummary production harvest adapters' -Tag 'Unit','Task15.60.c-f' {
  ...
  It 'exports the minimal production surface at immutable version 0.1.10' {
    (Get-Module ATAP.Utilities.BuildTooling.ContentSummary.PowerShell).Version.ToString() | Should -Be '0.1.10'
  ...
  It 'binds only the frozen controlled procedures and no direct DML' {
    $adapterText = @(
      [IO.File]::ReadAllText((Join-Path $script:moduleRoot 'public\New-ContentSummarySqlAdapterSet.ps1'))
      [IO.File]::ReadAllText((Join-Path $script:moduleRoot 'private\New-ContentSummarySqlAdapterSetCore.ps1'))
