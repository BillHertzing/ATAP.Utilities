...
#>
  ...
  if (-not (Get-Command -Name 'ConvertTo-BuildPromotionTierName' -CommandType Function -ErrorAction SilentlyContinue)) {
    $helperPath = Join-Path $PSScriptRoot 'ConvertTo-BuildPromotionTierName.ps1'
    if (Test-Path -LiteralPath $helperPath -PathType Leaf) {
      . $helperPath
    } else {
      throw "Required helper ConvertTo-BuildPromotionTierName was not found at '$helperPath'."
    }
  }
